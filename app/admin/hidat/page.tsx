...
import HiDatForm from "./components/hidat-form";
...
export default function Page() {
  ...
  const { data, isLoading, mutate } = hiDatServices.GetList(meta);
...
const { data:dataMaNguonGocs }=hiDatServices.GetMaNguonGoc()
  ...
  const handleChange = (res: any) => {
   ...
   useEffect(() => {
    setPermisson(getPermisson("hidat"));
  }, []);
  return (
    <>
...
<GridView.Table.Column style={{}}title="Ngày đưa vào sử dụng"sortKey="ngaySuDung"body={({ item }) => (<span>{formatDateTime(item.ngaySuDung)   }</span>)} />
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,hiDatServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      <HiDatForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
      <ConfirmationDialog />
    </>
  );
}
