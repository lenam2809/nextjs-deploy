...
export default function TangMoiDatForm({
  show,
  action,
  id,
  onClose,
  ...
  const { data, error, isLoading, mutate } = tangMoiDatServices.GetById(id!);
  const { data: dataDonVis } = tangMoiDatServices.GetDonVi();
  const { data: dataPhongBanChiuPhis } =
    tangMoiDatServices.GetPhongBanChiuPhi();
  const { data: dataPhongBanQuanLys } = tangMoiDatServices.GetPhongBanQuanLy();
  const { data: dataLoaiTaiSans } = tangMoiDatServices.GetLoaiTaiSan();
  const { data: dataNhomTaiSans } = tangMoiDatServices.GetNhomTaiSan();
  const { data: dataMaDiaBans } = tangMoiDatServices.GetMaDiaBan();
  const { data: dataMaMucDichs } = tangMoiDatServices.GetMaMucDich();
  const { data: dataThoiHanSuDungs } = tangMoiDatServices.GetThoiHanSuDung();
  const { data: dataMaNguonGocs } = tangMoiDatServices.GetMaNguonGoc();
  const { data: dataTrangThaiSuDungs } =
    tangMoiDatServices.GetTrangThaiSuDung();
  const { data: dataThoiDiemCoPhanHoas } =
    tangMoiDatServices.GetThoiDiemCoPhanHoa();
  const { data: dataPaSdDuocDuyets } = tangMoiDatServices.GetPaSdDuocDuyet();
  const { data: dataTinhTrangPasxs } = tangMoiDatServices.GetTinhTrangPasx();
  const { data: dataPasxDuocDuyets } = tangMoiDatServices.GetPasxDuocDuyet();
  const { data: dataTtHoSoPhapLys } = tangMoiDatServices.GetTtHoSoPhapLy();
  const { data: dataCapCongTrinhs } = tangMoiDatServices.GetCapCongTrinh();
  const [loading, setLoading] = useState(false);
  const onSubmit = async (values: any) => {
    setLoading(true);
    if (id) {
      try {
        await tangMoiDatServices.update(id, values);
      ...
      try {
        await tangMoiDatServices.create(values);
          ...
          {({ handleSubmit }) => (
            ...
            >
              ...
              <Modal.Body nameClass="grid-cols-12">
                <h2 className="">Thông tin quản lý</h2>
                ...
                <div className="col-span-4">
                  <TanetCheckbox
                    ...
                    name="daTinhSoLieu"
                  >
                    Đánh dấu tình trạng tính số liệu
                  </TanetCheckbox>
                </div>
                <div className="col-span-4">
                  <TanetInput
