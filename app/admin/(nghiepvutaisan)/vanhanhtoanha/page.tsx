...
import VanHanhToaNhaForm from "./components/vanhanhtoanha-form";
...
export default function Page() {
  ...
  const { data, isLoading, mutate } = vanHanhToaNhaServices.GetList(meta);
  ...
  const handleChange = (res: any) => {
   ...
   useEffect(() => {
    setPermisson(getPermisson("vanhanhtoanha"));
  }, []);
  return (
    <>
      <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
        ...
        >
          ...
          />
          <GridView.Table.Column style={{}}title="Tài sản id"sortKey="taiSanId"body={({ item }) => (<span>{item.taiSanId}</span>)} />
<GridView.Table.Column style={{}}title="Đơn vị vận hành tòa nhà"sortKey="donViVanHanhId"body={({ item }) => (<span>{item.donViVanHanhId}</span>)} />
<GridView.Table.Column style={{}}title="Thời gian vận hành từ ngày"sortKey="tuNgay"body={({ item }) => (<span>{formatDateTime(item.tuNgay)   }</span>)} />
<GridView.Table.Column style={{}}title="Thời gian vận hành đến ngày"sortKey="denNgay"body={({ item }) => (<span>{formatDateTime(item.denNgay)   }</span>)} />
          <GridView.Table.Column
            style={{ width: "10%" }}
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,vanHanhToaNhaServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      <VanHanhToaNhaForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
      <ConfirmationDialog />
    </>
  );
}
