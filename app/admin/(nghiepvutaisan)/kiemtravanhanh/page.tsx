...
import KiemTraVanHanhForm from "./components/kiemtravanhanh-form";
...
export default function Page() {
  ...
  const { data, isLoading, mutate } = kiemTraVanHanhServices.GetList(meta);
  ...
  const handleChange = (res: any) => {
   ...
   useEffect(() => {
    setPermisson(getPermisson("kiemtravanhanh"));
  }, []);
  return (
    <>
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,kiemTraVanHanhServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      <KiemTraVanHanhForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
      <ConfirmationDialog />
    </>
  );
}
