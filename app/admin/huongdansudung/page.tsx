...
import HuongDanSuDungForm from "./components/huongdansudung-form";
...
export default function Page() {
  ...
  const { data, isLoading, mutate } = huongDanSuDungServices.GetList(meta);
  ...
  const handleChange = (res: any) => {
   ...
   useEffect(() => {
    setPermisson(getPermisson("huongdansudung"));
  }, []);
  return (
    <>
...
<GridView.Table.Column style={{}}title="Tiêu đề"sortKey="tieuDe"body={({ item }) => (<span>{item.tieuDe}</span>)} />
            ...
            body={({ item }) => (
                <div className="flex flex-row">
                    ...
                    {permisson.per_Delete && <AiFillDelete
                      ...
                      title="Xóa"
                      onClick={() => delAction(item,huongDanSuDungServices,data,setMeta,meta,mutate)}
                    />
                }
              </div>             
            )}
      ...
      <HuongDanSuDungForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
      <ConfirmationDialog />
    </>
  );
}
